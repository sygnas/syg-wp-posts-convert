...
var sync = require('csv-parse/dist/esm/sync');
var convertRestToNormal = require('./convertRestToNormal.js');
var convertCustomToNormal = require('./convertCustomToNormal.js');
var removeTag = require('./helpers/removeTag.js');
var convertDate = require('./helpers/convertDate.js');
...
var WpPostsConvert = /*#__PURE__*/function () {
  ...
  function WpPostsConvert() {
    ...
    _rollupPluginBabelHelpers.classCallCheck(this, WpPostsConvert);
    ...
    var defaults = {
      ...
      // ヘルパー関数
      helpers: {
        removeTag: removeTag["default"],
        convertDate: convertDate["default"]
      }
    }; // 設定をマージ
  ...
  _rollupPluginBabelHelpers.createClass(WpPostsConvert, [{
    key: "start",
    value: function start(url) {
      ...
      return new Promise(function (resolve, reject) {
        // jsonを読み込む
        axios__default["default"].get(url).then(function (res) {
          // 受け取った記事一覧を変換し、
          // 指定されたエレメントに埋め込む
    ...
    value: function convertList(data) {
      ...
      data.forEach(function (post) {
        ...
        if (_this2.opt.type === WpPostsConvert.TYPE.REST) {
          wpData = convertRestToNormal["default"](post);
        } else {
          wpData = convertCustomToNormal["default"](post);
        } // テンプレートを適用
    ...
    value: function convertHelper(template, post) {
      ...
      while ((match = HELPER_REG.exec(template)) !== null) {
        ...
        var params = sync.parse(match[2], {
          columns: false
        })[0]; // ヘルパーの実行結果で置換する

        if (helpers && helpers[helperName]) {
          var result = helpers[helperName].apply(helpers, [post].concat(_rollupPluginBabelHelpers.toConsumableArray(params)));
...
_rollupPluginBabelHelpers.defineProperty(WpPostsConvert, "TYPE", {
  REST: "rest",
  CUSTOM: "custom"
});

exports["default"] = WpPostsConvert;
